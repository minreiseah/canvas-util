...
std::vector<Folder> listFoldersByCourse(CURL *curl, std::string courseID, std::string courseName);
...
int main(int argc, char *argv[]){
    std::printf("\033[1;36mDOWNLOADING VIDEOS\033[0m\n");
    ...
    for(auto &course : courses) {
        ...
        std::vector<Folder> folders = listFoldersByCourse(curl, course.second, course.first);
...
std::vector<Folder> listFoldersByCourse(CURL *curl, std::string courseID, std::string courseName) {
    ...
    // set query
    Json::Value query;
    query["queryParameters"]["folderID"] = courseID;
    ...
    for(auto &folder : root["d"]["Subfolders"]) {
        Folder data;
        data.id = folder["ID"].asString();
        data.name = folder["Name"].asString();
        data.path = TARGETDIR + courseName + "/lectures/" + data.name;
